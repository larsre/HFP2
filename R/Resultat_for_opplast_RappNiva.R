#' Data table for export of estimates - total
#' 
#' Generates a data table with estimates, confidence levels, brood sizes
#' etc., ready for import into HFP.
#' @param userID The user ID of the person analysing the data
#' @param Navn The name of the person analysing the data
#' @keywords table export total
#' @export
#' @examples
#' \dontrun{
#' Resultat_for_opplast_RappNiva(userID=16, Navn="Erlend Nilsen")
#' }

Resultat_for_opplast_RappNiva <- function(userID=16, Navn="Erlend Nilsen"){
  ...
  # Recruits/production
  temp[1,18] <- Rekrutt_all[1]
  temp[1,19] <- Rekrutt_all[1]-(1.96*Rekrutt_all[2])
  temp[1,20] <- Rekrutt_all[1]+(1.96*Rekrutt_all[2])
  temp[1,21] <- round(Rekrutt_all[2]/Rekrutt_all[1],2)  
